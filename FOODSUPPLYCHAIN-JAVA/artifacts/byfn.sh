...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd ./channel/crypto-config/peerOrganizations/manufacturer.supplychain.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yaml
  cd ./channel/crypto-config/peerOrganizations/middlemen.supplychain.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yaml
  cd ./channel/crypto-config/peerOrganizations/consumer.supplychain.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
...
# (``peer0.org1.supplychain.com`` & ``peer0.org2.supplychain.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  configtxgen -profile ThreeOrgsChannel -outputCreateChannelTx ./channel/mychannel.tx -channelID $CHANNEL_NAME
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel/ManufacturerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ManufacturerMSP
  if [ "$?" -ne 0 ]; then
  ...
  ./channel/MiddlemenMSPanchors.tx -channelID $CHANNEL_NAME -asOrg MiddlemenMSP
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate \
  ./channel/ConsumerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ConsumerMSP
  if [ "$?" -ne 0 ]; then
